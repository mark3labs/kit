...
<td>string</td>
<td><code>off</code></td>
<td>Extended thinking: off, none, minimal, low, medium, high. Levels a model does not accept are substituted with the nearest supported one at runtime.</td>
</tr>
<tr>
...
<td>string</td>
<td>—</td>
<td>Wire protocol for auto-routed providers: <code>openai</code>, <code>openai-compat</code>, <code>anthropic</code>, <code>google</code> (see <a href="#provider-overrides">Provider overrides</a>)</td>
</tr>
<tr>
...
<td>bool</td>
<td><code>false</code></td>
<td>Disable named agent discovery (<a href="/advanced/subagents#named-agents">built-ins and definition files</a>)</td>
</tr>
<tr>
<td><code>skill</code></td>
<td>list</td>
<td>—</td>
<td>Explicit skill files or directories to load (disables auto-discovery)</td>
</tr>
<tr>
<td><code>skills-dir</code></td>
<td>string</td>
<td>—</td>
<td>Scan this directory directly for skills (overrides auto-discovery; not treated as a parent of <code>.agents</code>/<code>.kit</code>)</td>
</tr>
<tr>
<td><code>skill-disable</code></td>
<td>list</td>
<td>—</td>
<td>Skill names to hide from the model catalog (still usable via <code>/&lt;name&gt;</code>)</td>
</tr>
</tbody>
</table>
...
<p>A few environment variables are read directly and have no <code>.kit.yml</code>
equivalent:</p>
<table>
<thead>
<tr>
...
<tbody>
<tr>
<td><code>KIT_SYSTEM_EXTENSIONS_DIR</code></td>
<td>Overrides the system-wide extension directories, separated by the platform list separator (<code>:</code> on Unix, <code>;</code> on Windows). Empty disables system-wide discovery. See <a href="/extensions/loading#system-wide-extensions">Loading Extensions</a>.</td>
</tr>
</tbody>
</table>
...
<span class="line"><span style="color:#22863A;--shiki-dark:#85E89D">    url</span><span style="color:#24292E;--shiki-dark:#E1E4E8">: </span><span style="color:#032F62;--shiki-dark:#9ECBFF">"https://mcp.example.com/search"</span></span>
<span class="line"></span>
<span class="line"><span style="color:#22863A;--shiki-dark:#85E89D">  pubmed</span><span style="color:#24292E;--shiki-dark:#E1E4E8">:</span></span>
<span class="line"><span style="color:#22863A;--shiki-dark:#85E89D">    type</span><span style="color:#24292E;--shiki-dark:#E1E4E8">: </span><span style="color:#032F62;--shiki-dark:#9ECBFF">remote</span></span>
<span class="line"><span style="color:#22863A;--shiki-dark:#85E89D">    url</span><span style="color:#24292E;--shiki-dark:#E1E4E8">: </span><span style="color:#032F62;--shiki-dark:#9ECBFF">"https://pubmed.mcp.example.com"</span></span>
<span class="line"><span style="color:#22863A;--shiki-dark:#85E89D">    noOAuth</span><span style="color:#24292E;--shiki-dark:#E1E4E8">: </span><span style="color:#005CC5;--shiki-dark:#79B8FF">true</span><span style="color:#6A737D;--shiki-dark:#6A737D">  # skip OAuth for public servers</span></span>
...
<span class="line"><span style="color:#24292E;--shiki-dark:#E1E4E8">      - </span><span style="color:#032F62;--shiki-dark:#9ECBFF">"ApiKey: \${env://API_KEY}"</span><span style="color:#6A737D;--shiki-dark:#6A737D">              # required env var</span></span>
<span class="line"><span style="color:#24292E;--shiki-dark:#E1E4E8">      - </span><span style="color:#032F62;--shiki-dark:#9ECBFF">"X-Tenant: \${env://TENANT_ID:-default}"</span><span style="color:#6A737D;--shiki-dark:#6A737D"> # with fallback default</span></span>
<span class="line"></span>
<span class="line"><span style="color:#22863A;--shiki-dark:#85E89D">  builds</span><span style="color:#24292E;--shiki-dark:#E1E4E8">:</span></span>
<span class="line"><span style="color:#22863A;--shiki-dark:#85E89D">    type</span><span style="color:#24292E;--shiki-dark:#E1E4E8">: </span><span style="color:#032F62;--shiki-dark:#9ECBFF">remote</span></span>
<span class="line"><span style="color:#22863A;--shiki-dark:#85E89D">    url</span><span style="color:#24292E;--shiki-dark:#E1E4E8">: </span><span style="color:#032F62;--shiki-dark:#9ECBFF">"https://builds.mcp.example.com"</span></span>
<span class="line"><span style="color:#22863A;--shiki-dark:#85E89D">    tasksMode</span><span style="color:#24292E;--shiki-dark:#E1E4E8">: </span><span style="color:#032F62;--shiki-dark:#9ECBFF">always</span><span style="color:#6A737D;--shiki-dark:#6A737D">  # always run tools/call as async tasks (Phase 1 MVP)</span></span></code></pre>
...
<tr>
<td><code>noOAuth</code></td>
<td>bool</td>
<td>Skip OAuth for this server (for public servers that don't require auth)</td>
</tr>
<tr>
<td><code>headers</code></td>
<td>list of strings</td>
<td>HTTP headers to attach to every request, each as a <code>"Key: Value"</code> string. Values support env-substitution: <code>\${env://VAR}</code> or <code>\${env://VAR:-default}</code>.</td>
</tr>
<tr>
<td><code>tasksMode</code></td>
<td>string</td>
<td>When to augment <code>tools/call</code> with MCP task metadata: <code>auto</code> (default — only when the server advertises task support), <code>never</code>, or <code>always</code>. See <a href="#mcp-tasks-long-running-tools">MCP tasks</a>.</td>
</tr>
</tbody>
</table>
<p>A legacy format with <code>transport</code>, <code>args</code>, and <code>env</code> fields is also supported; <code>headers</code> works in both the current and legacy formats.</p>
<p>For a one-off server without a config file, pass <code>--mcp 'name=command args...'</code>
or <code>--mcp 'name=https://... -H "Key: Value"'</code> on the command line. See
<a href="/cli/flags#one-off-mcp-servers">CLI flags</a>.</p>
<h3 id="mcp-tasks-long-running-tools"><a class="heading-anchor" aria-hidden="" tabindex="-1" href="#mcp-tasks-long-running-tools"><span class="icon icon-link"></span></a>MCP tasks (long-running tools)</h3>
<p>Kit advertises <a href="https://modelcontextprotocol.io/specification/2025-11-25/basic/utilities/tasks">MCP task support</a>
during <code>initialize</code> so servers can respond to <code>tools/call</code> with a
<code>CreateTaskResult</code> (a task ID + <code>working</code> status) instead of blocking until
the operation finishes. Kit then polls <code>tasks/get</code> / <code>tasks/result</code> until the
task reaches a terminal state, and best-effort <code>tasks/cancel</code>s on context
cancellation.</p>
<p>This avoids HTTP/SSE proxy timeouts on long builds, deploys, and batch jobs,
and lets the user/agent abort cleanly with Ctrl-C.</p>
<p><strong>Per-server <code>tasksMode</code>:</strong></p>
<table>
<thead>
<tr>
<th>Value</th>
...
<td>Augment <code>tools/call</code> with task metadata only when the server advertised <code>tasks/toolCalls</code> capability. Servers that don't advertise it run synchronously, exactly as before.</td>
</tr>
<tr>
<td><code>never</code></td>
<td>Always issue <code>tools/call</code> synchronously, regardless of server capability.</td>
</tr>
<tr>
<td><code>always</code></td>
<td>Always opt into task augmentation, even when the server didn't advertise the capability. The server may still respond synchronously — this just expresses client intent unconditionally.</td>
</tr>
</tbody>
</table>
<p>Defaults are safe: any existing MCP server keeps its previous behaviour
bit-for-bit. SDK consumers can also override the mode programmatically and
plug in a progress callback — see <a href="/sdk/options#mcp-tasks">SDK options</a>.</p>
...
<p>Declare or patch providers in the model registry with the <code>providers</code> section. Use it to fix a wire protocol the model database routes wrongly, or to define an entirely new provider (such as an internal LLM gateway) that the database doesn't know about:</p>
<pre class="shiki shiki-themes github-light github-dark" style="background-color:#fff;--shiki-dark-bg:#24292e;color:#24292e;--shiki-dark:#e1e4e8" tabindex="0"><code><span class="line"><span style="color:#22863A;--shiki-dark:#85E89D">providers</span><span style="color:#24292E;--shiki-dark:#E1E4E8">:</span></span>
...
<h3 id="provider-override-fields"><a class="heading-anchor" aria-hidden="" tabindex="-1" href="#provider-override-fields"><span class="icon icon-link"></span></a>Provider override fields</h3>
<table>
<thead>
<tr>
<th>Field</th>
...
<td>string</td>
<td>Display name for the provider</td>
</tr>
<tr>
<td><code>wire</code></td>
<td>string</td>
<td>Wire protocol: <code>openai</code> (Responses API), <code>openai-compat</code> (chat completions), <code>anthropic</code>, or <code>google</code></td>
</tr>
<tr>
...
<td>list</td>
<td>Environment variable names tried in order when resolving the API key</td>
</tr>
<tr>
<td><code>headers</code></td>
<td>map</td>
<td>Default HTTP headers added to every request</td>
</tr>
</tbody>
</table>
<p>All fields are optional. Non-empty fields override the model database; unset fields inherit the database values. An invalid <code>wire</code> value causes the whole override to be skipped with a warning. Wire resolution precedence: <code>--provider-wire</code> flag &gt; <code>providers</code> config &gt; the database's own routing.</p>
<p>Unlike <a href="#custom-models">custom models</a> (which define individual models under the <code>custom</code> provider, always on the OpenAI-compatible wire), provider overrides operate at the provider level and support all four wire protocols. See <a href="/providers#auto-routed-providers">Auto-routed providers</a> for how routing works.</p>
...
<p>Define custom models in your <code>.kit.yml</code> for use with the <code>custom</code> provider. This is useful for self-hosted models or API endpoints not in the built-in database (for non-OpenAI wire protocols or provider-level declarations, use <a href="#provider-overrides">provider overrides</a> instead):</p>
...
<span class="line"><span style="color:#22863A;--shiki-dark:#85E89D">    apiModelName</span><span style="color:#24292E;--shiki-dark:#E1E4E8">: </span><span style="color:#032F62;--shiki-dark:#9ECBFF">"gpt-4-turbo"</span></span>
...
<tr>
<td><code>apiModelName</code></td>
<td>string</td>
<td>No</td>
<td>Overrides the model identifier sent in API requests; defaults to the config key</td>
</tr>
<tr>
...
<p>Settings from <code>modelSettings</code> and <code>customModels.params</code> act as model-level defaults — explicit CLI flags, <code>KIT_*</code> environment variables, global config values, and SDK <code>Options.*</code> fields all take precedence over them.</p>
...
<h3 id="precedence-summary"><a class="heading-anchor" aria-hidden="" tabindex="-1" href="#precedence-summary"><span class="icon icon-link"></span></a>Precedence summary</h3>
<p>For the generation and provider parameters documented above, the resolved value at runtime comes from the first source that sets it:</p>
<ol>
<li>CLI flag (e.g. <code>--max-tokens</code>, <code>--temperature</code>, <code>--provider-api-key</code>)</li>
<li>SDK <code>Options.X</code> when embedding Kit as a library (<code>kit.Options.MaxTokens</code>, <code>Temperature</code>, <code>ProviderAPIKey</code>, etc.)</li>
<li><code>KIT_*</code> environment variable (<code>KIT_MAX_TOKENS</code>, <code>KIT_TEMPERATURE</code>, ...)</li>
<li><code>.kit.yml</code> / <code>.kit.yaml</code> / <code>.kit.json</code> (project-local, then global)</li>
<li>Per-model defaults (<code>modelSettings[provider/model]</code> / <code>customModels[...].params</code>)</li>
<li>Provider-level defaults (e.g. Anthropic's own temperature default)</li>
<li>SDK last-resort floor — currently an 8192 output-token ceiling matching the CLI <code>--max-tokens</code> default, auto-raised per-model up to 32768 when the model's catalog ceiling is higher</li>
</ol>
<p>See the <a href="/sdk/options">SDK options reference</a> for the full list of <code>kit.Options</code> fields that map to these keys.</p>
