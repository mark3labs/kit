...
<td>—</td>
<td><code>false</code></td>
<td>Choose a working directory with a picker before starting. The daemon uses this flag for any <a href="/cli/commands#detachable-sessions">detachable session</a> that has no directory of its own — <code>kit attach --new</code>, or a remote client</td>
</tr>
<tr>
<td><code>--no-daemon</code></td>
<td>—</td>
<td><code>false</code></td>
<td>Run in this terminal even when the kit daemon could host a <a href="/advanced/remote-sessions#detachable-by-default">detachable session</a>. <code>KIT_NO_DAEMON=1</code> does the same; <code>daemon-mode</code> in the config file is the standing preference</td>
</tr>
<tr>
...
<td>Shell the shell tool and the interactive <code>!</code> commands run through: the shell plus its own leading arguments, e.g. <code>/bin/dash</code> or <code>"busybox ash"</code></td>
</tr>
<tr>
<td><code>--no-core-tools</code></td>
<td>—</td>
<td><code>false</code></td>
<td>Disable all built-in core tools</td>
</tr>
<tr>
<td><code>--include-core-tools</code></td>
<td>—</td>
<td>—</td>
<td>Comma-separated list of core tools to include (mutually exclusive with <code>--exclude-core-tools</code>)</td>
</tr>
<tr>
<td><code>--exclude-core-tools</code></td>
<td>—</td>
<td>—</td>
<td>Comma-separated list of core tools to exclude (mutually exclusive with <code>--include-core-tools</code>)</td>
</tr>
<tr>
<td><code>--mcp</code></td>
<td>—</td>
<td>—</td>
<td>Add an MCP server for this run (repeatable). <code>name=command args...</code> starts a local stdio server; <code>name=https://... [-H "Key: Value"]</code> connects to a remote server</td>
</tr>
</tbody>
</table>
<h3 id="one-off-mcp-servers"><a class="heading-anchor" aria-hidden="" tabindex="-1" href="#one-off-mcp-servers"><span class="icon icon-link"></span></a>One-off MCP servers</h3>
<p><code>--mcp</code> adds a server without a config file. Each value is <code>name=spec</code>. When
<code>spec</code> starts with <code>http://</code> or <code>https://</code> Kit connects to a remote server;
otherwise <code>spec</code> is a command line for a local stdio server. Quotes and
backslashes group arguments the way a shell does, so an argument can contain
spaces.</p>
<pre class="shiki shiki-themes github-light github-dark" style="background-color:#fff;--shiki-dark-bg:#24292e;color:#24292e;--shiki-dark:#e1e4e8" tabindex="0"><code><span class="line"><span style="color:#6F42C1;--shiki-dark:#B392F0">kit</span><span style="color:#005CC5;--shiki-dark:#79B8FF"> --mcp</span><span style="color:#032F62;--shiki-dark:#9ECBFF"> 'browser=lightpanda mcp'</span><span style="color:#032F62;--shiki-dark:#9ECBFF"> "open example.com and read the title"</span></span>
<span class="line"><span style="color:#6F42C1;--shiki-dark:#B392F0">kit</span><span style="color:#005CC5;--shiki-dark:#79B8FF"> --mcp</span><span style="color:#032F62;--shiki-dark:#9ECBFF"> 'docs=https://mcp.example.com/mcp'</span><span style="color:#032F62;--shiki-dark:#9ECBFF"> "..."</span></span>
<span class="line"><span style="color:#6F42C1;--shiki-dark:#B392F0">kit</span><span style="color:#005CC5;--shiki-dark:#79B8FF"> --mcp</span><span style="color:#032F62;--shiki-dark:#9ECBFF"> 'fs=npx -y @modelcontextprotocol/server-filesystem "/my dir"'</span><span style="color:#005CC5;--shiki-dark:#79B8FF"> \\</span></span>
<span class="line"><span style="color:#005CC5;--shiki-dark:#79B8FF">    --mcp</span><span style="color:#032F62;--shiki-dark:#9ECBFF"> 'browser=lightpanda mcp'</span><span style="color:#032F62;--shiki-dark:#9ECBFF"> "..."</span></span></code></pre>
<h4 id="headers-for-remote-servers"><a class="heading-anchor" aria-hidden="" tabindex="-1" href="#headers-for-remote-servers"><span class="icon icon-link"></span></a>Headers for remote servers</h4>
<p>A remote spec can carry HTTP headers after the URL, curl style. Use <code>-H</code> or
<code>--header</code>, once per header, with the header as one <code>"Key: Value"</code> string.
Header values support the same env-substitution as the config file, so an API
key stays out of the command line and out of the shell history:</p>
<pre class="shiki shiki-themes github-light github-dark" style="background-color:#fff;--shiki-dark-bg:#24292e;color:#24292e;--shiki-dark:#e1e4e8" tabindex="0"><code><span class="line"><span style="color:#6F42C1;--shiki-dark:#B392F0">kit</span><span style="color:#005CC5;--shiki-dark:#79B8FF"> --mcp</span><span style="color:#032F62;--shiki-dark:#9ECBFF"> 'docs=https://mcp.example.com/mcp -H "Authorization: Bearer \${env://API_KEY}"'</span><span style="color:#032F62;--shiki-dark:#9ECBFF"> "..."</span></span>
<span class="line"><span style="color:#6F42C1;--shiki-dark:#B392F0">kit</span><span style="color:#005CC5;--shiki-dark:#79B8FF"> --mcp</span><span style="color:#032F62;--shiki-dark:#9ECBFF"> 'docs=https://mcp.example.com/mcp -H "X-Api-Key: \${env://KEY}" -H "X-Tenant: acme"'</span><span style="color:#032F62;--shiki-dark:#9ECBFF"> "..."</span></span>
<span class="line"><span style="color:#6F42C1;--shiki-dark:#B392F0">kit</span><span style="color:#005CC5;--shiki-dark:#79B8FF"> --mcp</span><span style="color:#032F62;--shiki-dark:#9ECBFF"> 'docs=https://mcp.example.com/mcp --header="X-Api-Key: secret"'</span><span style="color:#032F62;--shiki-dark:#9ECBFF"> "..."</span></span></code></pre>
<p><code>\${env://VAR}</code> fails at startup when <code>VAR</code> is not set; use
<code>\${env://VAR:-default}</code> for an optional value. Headers apply to remote servers
only — after a local command, <code>-H</code> is passed to that command unchanged.</p>
<p>Headers are the credential for that server, so Kit switches OAuth off for it.
Otherwise Kit would try dynamic client registration first and fail before it
sends the headers. Two switches override this:</p>
<table>
<thead>
<tr>
<th>Argument</th>
...
<td>Keep OAuth on, even with headers (for a server that needs OAuth plus a routing header)</td>
</tr>
<tr>
<td><code>--no-oauth</code></td>
<td>Switch OAuth off without headers (for a public server)</td>
</tr>
</tbody>
</table>
<pre class="shiki shiki-themes github-light github-dark" style="background-color:#fff;--shiki-dark-bg:#24292e;color:#24292e;--shiki-dark:#e1e4e8" tabindex="0"><code><span class="line"><span style="color:#6F42C1;--shiki-dark:#B392F0">kit</span><span style="color:#005CC5;--shiki-dark:#79B8FF"> --mcp</span><span style="color:#032F62;--shiki-dark:#9ECBFF"> 'docs=https://mcp.example.com/mcp -H "X-Tenant: acme" --oauth'</span><span style="color:#032F62;--shiki-dark:#9ECBFF"> "..."</span></span>
<span class="line"><span style="color:#6F42C1;--shiki-dark:#B392F0">kit</span><span style="color:#005CC5;--shiki-dark:#79B8FF"> --mcp</span><span style="color:#032F62;--shiki-dark:#9ECBFF"> 'pubmed=https://mcp.example.com/mcp --no-oauth'</span><span style="color:#032F62;--shiki-dark:#9ECBFF"> "..."</span></span></code></pre>
<p>A flag server with the same name as a <code>mcpServers</code> entry in <code>.kit.yml</code>
replaces that entry for the run. For <code>environment</code>, <code>allowedTools</code> and the
other per-server fields, use the <a href="/configuration#mcp-server-configuration">config file</a>.</p>
...
<td>—</td>
<td>—</td>
<td>Hide a skill from the model catalog by name (repeatable); still usable via <code>/&lt;name&gt;</code></td>
</tr>
<tr>
