...
<td>Switch model or open model selector. The selector lists every model in the catalogue; models whose provider has no credentials are dimmed and cannot be selected — use <code>/connect</code> to add a key. Search ranks by match quality alone, so dimmed models stay findable (e.g. type <code>openrouter/sonnet</code>).</td>
</tr>
<tr>
...
background daemon. See <a href="/advanced/remote-sessions">Remote sessions</a> for
the full picture.</p>
<p><strong>When the daemon is running, plain <code>kit</code> is already detachable.</strong> There
is no separate command to learn: the session starts in the directory you
ran it from, with the arguments you gave it.</p>
<pre class="shiki shiki-themes github-light github-dark" style="background-color:#fff;--shiki-dark-bg:#24292e;color:#24292e;--shiki-dark:#e1e4e8" tabindex="0"><code><span class="line"><span style="color:#6F42C1;--shiki-dark:#B392F0">kit</span><span style="color:#032F62;--shiki-dark:#9ECBFF"> daemon</span><span style="color:#032F62;--shiki-dark:#9ECBFF"> service</span><span style="color:#032F62;--shiki-dark:#9ECBFF"> install</span><span style="color:#6A737D;--shiki-dark:#6A737D">        # Start the daemon once (systemd user service)</span></span>
<span class="line"></span>
<span class="line"><span style="color:#005CC5;--shiki-dark:#79B8FF">cd</span><span style="color:#032F62;--shiki-dark:#9ECBFF"> ~/project</span><span style="color:#24292E;--shiki-dark:#E1E4E8"> &amp;&amp; </span><span style="color:#6F42C1;--shiki-dark:#B392F0">kit</span><span style="color:#6A737D;--shiki-dark:#6A737D">               # A detachable session, right here</span></span>
...
<p>Kit falls back to running in your terminal when no daemon is running, for
a one-shot prompt (<code>kit "..."</code>), for <code>--quiet</code>/<code>--json</code>, when stdin or
stdout is not a terminal, and for <code>--no-daemon</code>. See <a href="/advanced/remote-sessions#detachable-by-default">Detachable by
default</a> for the exact
rule and the <code>daemon-mode</code> setting.</p>
<p><code>kit attach</code> is the command for choosing <em>which</em> session — going back to
one, or starting one somewhere else. It starts a daemon automatically if
none is running.</p>
...
<span class="line"><span style="color:#6F42C1;--shiki-dark:#B392F0">kit</span><span style="color:#032F62;--shiki-dark:#9ECBFF"> ls</span><span style="color:#005CC5;--shiki-dark:#79B8FF"> --all</span><span style="color:#6A737D;--shiki-dark:#6A737D">                      # Include sessions on every paired host</span></span>
<span class="line"></span>
<span class="line"><span style="color:#6F42C1;--shiki-dark:#B392F0">kit</span><span style="color:#005CC5;--shiki-dark:#79B8FF"> --no-daemon</span><span style="color:#6A737D;--shiki-dark:#6A737D">                   # Run in this terminal, daemon or no daemon</span></span></code></pre>
...
<p>Where a new session starts depends on how you asked for it:</p>
<table>
<thead>
<tr>
<th>How you started it</th>
...
<tr>
<td><code>kit</code></td>
<td>Where you ran it — no picker</td>
</tr>
<tr>
<td><code>Ctrl+] c</code></td>
<td>The same directory as the session you were in</td>
</tr>
<tr>
<td><code>kit attach</code>, <code>kit attach --new</code></td>
<td>The directory picker, opened where you ran it</td>
</tr>
<tr>
<td>A remote client (<code>kit remote --host</code>)</td>
<td>The directory picker, opened in the host's home directory</td>
</tr>
</tbody>
</table>
<p>A remote client never sends a directory: it names nothing that exists on
the host. <code>--new</code> skips the <em>session</em> picker, not the directory one.</p>
<p>You can also run the directory picker outside a session with
<a href="/cli/flags"><code>kit --pick-dir</code></a>; the daemon uses that same flag whenever a
session has no directory to start in.</p>
...
colors</a>.</p>
<p>A local client also forwards its working directory and an allowlist of
environment variables — <code>PATH</code>, <code>HOME</code>, <code>SHELL</code>, <code>EDITOR</code>, the locale,
proxy settings and the provider credentials — so a hosted session finds
the same tools and keys your shell has. Remote clients never do.</p>
